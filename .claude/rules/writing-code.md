...
**Workflow adapter sweep rule** — some adapters (e.g. `WebContentCreateWorkflowOperationAdapter`) delegate through an intermediate `*Request` helper class that constructs `*BatchSpec`. When a `*BatchSpec` gains a new field, sweep **both** the `*WorkflowOperationAdapter` and any `*Request` helper in the same package. Touching only the adapter leaves the `*Request` constructor call mismatched, causing a compile failure that is otherwise non-obvious.
